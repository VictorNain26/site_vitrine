  ...
  return (
    <>
      <div className="flex flex-col items-center my-11 mx-8">
        { data.isPlaying ? (
            <>
              <p className="text-3xl text-red-700 text-center fw-bold mb-7 animate-pulse">En Direct</p>
              <p className="text-3xl text-center">{data.title}</p>
              <p className="text-3xl italic text-center">{data.artist}</p>
        ...
        <h3 className={`inline rounded text-3xl cursor-pointer my-8 p-4 ${showTracks ? "bg-slate-950" : ""} ${data.isPlaying ? "" : "hidden"}`} onClick={toggleTracks}>Dernières écoutes</h3>
      </div>
    </>
  )
}
