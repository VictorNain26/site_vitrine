...
export default function Home() {
  ...
  return (
    <>
      ...
      <section className='fixed top-0 bg-slate-900 py-11 text-center lg:right-14 lg:text-left'> 
        <h3 className="text-3xl mb-8">Les 5 derniers...</h3>

        <div className="mx-8 lg:mx-0 text-left my-4">
          { data.recentlyPlayed.items.map((item) => (
            <Link key={item.track.id} href={`${item.track.externalURL.spotify}`} target="_blank" rel="noopener noreferrer" className="flex items-center p-4 rounded hover:bg-slate-950">
              <Image src={item.track.album.images[0].url} alt={item.track.title} width={60} height={60} className='rounded me-4' />
